...
class Disk(object):
    ...
    def map_partitions(self):
        ...
        """
        logging.info('Creating loop devices corresponding to the created partion')
        self.vm.add_clean_cb(lambda : self.unmap(ignore_fail=True))
	kpartx_output = run_cmd('kpartx', '-v','-a', self.filename)
        logging.info("Joseph Here is the output from kpartx (%s)" % kpartx_output)
        #print "Check now Joseph"
	time.sleep(5)
	parts = []
    ...
    def unmap(self, ignore_fail=False):
        ...
        while tries < max_tries:
            try:
                run_cmd('kpartx', '-d', self.filename)
                break
            except:
                pass
            tries += 1
