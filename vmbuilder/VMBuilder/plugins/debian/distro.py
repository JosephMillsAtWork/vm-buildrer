...
class Debian(Distro):
    name = 'Debian'
    arg = 'debian'
    suites = ['wheezy', 'jessie', 'sid', 'stretch' ]
    ...
    def register_options(self):
        ...
        group.add_setting('hostname', default='debian', help='Set NAME as the hostname of the guest. Default: debian. Also uses this name as the VM name.')

        group = self.setting_group('Installation options')
        group.add_setting('suite', default='jessie', help='Suite to install. Valid options: %s [default: %%default]' % ' '.join(self.suites))
        ...
        group.add_setting('mirror', metavar='URL', help='Use Debian mirror at URL instead of the default, which is http://http.debian.net/debian/')
        group.add_setting('proxy', metavar='URL', help='Use proxy at URL for cached packages')
        group.add_setting('install-mirror', metavar='URL', help='Use Debian mirror at URL for the installation only. Apt\'s sources.list will still use default or URL set by --mirror')
        group.add_setting('security-mirror', metavar='URL', help='Use Debian security mirror at URL instead of the default, which is http://security.debian.org/')
        ...
        group.add_setting('components', type='list', metavar='COMPS', help='A comma seperated list of distro components to include (e.g. main,contrib,non-free).')
    ...
    def set_defaults(self):
        ...
        if arch == 'lpia':
            self.set_setting_default('mirror', 'http://http.debian.net/debian/')
            self.set_setting_default('security-mirror', 'http://security.debian.org/')
        else:
            self.set_setting_default('mirror', 'http://http.debian.net/debian/')
            self.set_setting_default('security-mirror', 'http://security.debian.org/')

        self.set_setting_default('components',  ['contrib', 'main', 'non-free'])
    ...
    def preflight_check(self):
        ...
        if not components:
            self.set_config_value_list = ['main', 'contrib', 'non-free']
        else:
