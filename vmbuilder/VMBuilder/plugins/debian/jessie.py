...
#
from   VMBuilder.plugins.debian.wheezy import Wheezy
class Jessie(Wheezy):
    ...
    #def kernel_name(self):
        ...
        #return self.kernel_version 'linux-image-%s-%s' % (self.kernel_version, flavour or self.default_flavour[arch])'
...
#class Jessie(suite.Suite):
    #updategrub = "/usr/sbin/update-grub"
    #grubroot = "/usr/lib/grub"
    #valid_flavours = { 'i386' :  ['686-pae', '686', '686-smp', 'k7', 'k7-smp', 'server', 'server-bigiron'],
                       #'amd64' : ['amd64-generic', 'amd64-k8', 'amd64-k8-smp', 'amd64-server', 'amd64-xeon'],
			#'armhf' : ['armv7'] }
    #default_flavour = { 'i386' : '686-pae', 'amd64' : 'amd64', 'armhf' : 'armv7' }
    #disk_prefix = 'hd'
    ...
    #def kernel_name(self):
        #flavour = self.context.get_setting('flavour')
        #arch = self.context.get_setting('arch')
        #return 'linux-image-3.16.0-4-%s' % (flavour or self.default_flavour[arch],)
    ...
    #def install_fstab(self, disks, filesystems):
        #self.install_from_template('/etc/fstab', 'etch_fstab', { 'parts' : disk.get_ordered_partitions(disks), 'prefix' : self.disk_prefix })
    ...
    #def install_grub(self, chroot_dir):
        ...
        ##print "Joseph Look now to see if grub got installed !"
	##time.sleep(2)
